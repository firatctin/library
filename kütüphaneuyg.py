...
def kitapara():
    ...
    #Metin kutularına girilmiş olan veriyi alıyoruz
    ...
    con = sqlite3.connect("kitaplık.db")
    cursor = con.cursor()
    #Hangi text alanından veri geldiğini kontrol ediyoruz...
    if yazar != "" and isim != "":
        sorgu = '''SELECT * FROM kitaplar WHERE isim LIKE "%{}%" AND yazarismi LIKE "%{}%"'''.format(isim,yazar)
    elif isim == "":
        sorgu = '''SELECT * FROM kitaplar WHERE yazarismi LIKE "%{}%"'''.format(yazar)
    elif yazar == "":
        
        sorgu = '''SELECT * FROM kitaplar WHERE isim LIKE "%{}%"'''.format(isim)
    cursor.execute(sorgu)
    ...
    if len(sonuçlar) >0:
        ...
        messagebox.showerror("Başarısız!","Sonuç bulunamadı...")
